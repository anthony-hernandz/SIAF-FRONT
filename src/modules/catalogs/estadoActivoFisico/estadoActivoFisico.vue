...
const headers = ref([
  { title: 'Estado físico del Activo', align: 'start', key: 'estadoActivoFisico' },
...
const reglasestadoActivoFisico = [
  v => !!v || 'El estado físico del activo es obligatorio',
...
const agregarestadoActivoFisico = async () => {
  ...
  if (valid) {
    ...
    const newestadoActivoFisico = {
      estadoActivoFisico: estadoActivoFisico.value,
      register_by: 'Usuario Logueado', // Esto vendría del backend
      estado: 'INACTIVO', // por defecto
      isNew: true
    }
    items.value.push(newestadoActivoFisico)
...
const filteredItems = computed(() => {
  ...
  if (search.value.length >= 3) {
    ...
    filtered = filtered.filter(item =>
      item.estadoActivoFisico.toLowerCase().includes(searchTerm)
    );
  }
  return filtered;
});
...
const handleDelete = (item) => {
  ...
  items.value = items.value.filter(i => i.estadoActivoFisico !== item.estadoActivoFisico)
}
...
const confirmDeactivate = async () => {
  ...
  const index = items.value.findIndex(i => i.estadoActivoFisico === currentItem.value.estadoActivoFisico);
...
onMounted(async () => {
  ...
  await new Promise(resolve => setTimeout(resolve, 1000));
  items.value = [
    {estadoActivoFisico: 'compra', register_by: 'Aquiles Vengo', estado: 'ACTIVO' },
    {estadoActivoFisico: 'Donación', register_by: 'Marta Rillo', estado: 'INACTIVO' },
    {estadoActivoFisico: 'Pertura', register_by: 'Ana Lisis', estado: 'ACTIVO' },
    {estadoActivoFisico: 'compra dos', register_by: 'Ana Lisis', estado: 'INACTIVO' },
      ...
      <v-container fluid class="mb-8">
        <v-row justify="center" :class="display.xs || display.sm || display.md ? 'mb-8' : ''">
          <v-col cols="12" xl="4" lg="4" sm="12" md="4">
            ...
            >
              <app-right-from-catalaog-component @submit="agregarestadoActivoFisico">
                <template #myCatalog>
                  <v-form ref="formRef">
                    <v-text-field
                      v-model="estadoActivoFisico"
                      variant="solo"
                      label="Estado físico del activo: *"
                      :rules="reglasestadoActivoFisico"
                    ></v-text-field>
                  </v-form>
                </template>
              ...
              <v-row justify="center">
                ...
                <v-col cols="12" xl="12" lg="10" sm="12" md="12" xs="12">
                  <v-text-field
                    ...
                    variant="solo"
                    label="Buscar"
                    placeholder="Ingrese estado físico del activo"
